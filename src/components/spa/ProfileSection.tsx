                ...
                <div className="flex flex-col items-center gap-10 mb-8 z-10">
                    ...
                    <div className="flex flex-col items-center">
                        <motion.div
                            ...
                            className="w-24 h-24 rounded-2xl bg-gradient-to-tr from-blue-600 to-indigo-600 shadow-2xl flex items-center justify-center text-4xl font-black border-4 border-[var(--color-surface)] text-white relative mb-4"
                        >
                            {MOCK_USER.avatar}
                        ...
                        <AnimatePresence mode="popLayout" initial={false}>
                            {activeTab === "registered" ? (
                                ...
                                <motion.div
                                    ...
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
                                    className="py-12 text-center text-[var(--color-text-muted)] bg-[var(--color-surface)]/30 border border-dashed border-[var(--color-border)] rounded-2xl"
                                >
                ...
                <div className="bg-gradient-to-br from-indigo-900/30 to-blue-900/20 border border-indigo-500/20 rounded-2xl p-8 relative overflow-hidden shadow-2xl">
                    ...
                    <div className="relative z-10">
                        <h3 className="font-black text-[22px] mb-3 flex items-center gap-3">
                            <Award size={28} className="text-indigo-400" /> Campus Passport
                        </h3>
                        <p className="text-[14px] text-[var(--color-text-muted)] leading-relaxed mb-8 font-medium">
                            Your engagement determines career readiness. Build an investor-grade digital identity.
                        </p>
                        <button className="py-4 px-8 rounded-xl bg-indigo-600 text-white shadow-xl text-[15px] font-black active:scale-95 transition-all">
                            Export Digital Profile
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl overflow-hidden divide-y divide-[var(--color-border)] shadow-xl">
                    <div className="flex items-center justify-between p-6 px-8">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-xl bg-[var(--color-surface-elevated)] flex items-center justify-center border border-[var(--color-border)]">
                    ...
                    <div className="flex items-center justify-between p-6 px-8">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex flex-col text-left">
                                ...
                                <span className="text-[12px] text-[var(--color-text-muted)] font-medium">Auditory feedback</span>
                            </div>
                        </div>
                        <button
                            onClick={() => {
...
function StatCard({ label, value, color }: { label: string, value: number, color: 'blue' | 'purple' }) {
    return (
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-5 flex items-center justify-between relative overflow-hidden group shadow-sm">
            <div className={`absolute top-0 right-0 w-20 h-20 rounded-bl-3xl transition-colors ${color === 'blue' ? 'bg-blue-500/5 group-hover:bg-blue-500/10' : 'bg-purple-500/5 group-hover:bg-purple-500/10'}`} />
            <span className="text-[12px] font-black text-[var(--color-text-muted)] uppercase tracking-widest text-left opacity-80">{label}</span>
            <span className={`text-3xl font-black ${color === 'blue' ? 'text-blue-400' : 'text-purple-400'}`}>{value}</span>
        </div>
    );
}
